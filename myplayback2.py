...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def playback_btn_onclick(self):
        if not self.playbackID:
            ...
            if self.record_count >= 2:
                end_time = self.record_infos[
                    self.record_count - 2].endtime  # 最后一个录像，也就是self.record_infos[self.record_count-1]的endtime可能是0：0:0，所以取倒数第二个的结束时间
            else:
    ...
    def download_btn_onclick(self):
        if not self.downloadID:
            ...
            stream_type = self.StreamTyp_comboBox.currentIndex()  # 获取视频码流
            self.set_stream_type(stream_type)  # 设置视频码流

            start_date = self.Start_dateTimeEdit.date()  # 开始时间
            start_time = self.Start_dateTimeEdit.time()  # 从组件中取日期和时间
            startDateTime = NET_TIME()  # 实例网络事件对象
            ...
            nchannel = self.Channel_comboBox.currentIndex()  # 取视频通道
